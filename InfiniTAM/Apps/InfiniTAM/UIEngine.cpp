...
void UIEngine::glutDisplayFunction()
{
	...
	if (uiEngine->mainEngine->activeScene_idx == 1)
		...
		uiEngine->mainEngine->GetImage(uiEngine->outImage[1], uiEngine->outImageType[1], &uiEngine->freeviewPose, &uiEngine->freeviewIntrinsics);

	for (int w = 2; w < NUM_WIN; w++) uiEngine->mainEngine->GetImage(uiEngine->outImage[w], uiEngine->outImageType[w]);
...
void UIEngine::glutKeyUpFunction(unsigned char key, int x, int y)
{
	...
	{
	case 'y':
		uiEngine->mainEngine->saveSceneBeforeSwitching();
		...
		uiEngine->mainEngine->switchActiveScene();
	...
	case 'f':
		...
		if (uiEngine->freeviewActive)
		{
			...
			uiEngine->outImageType[2] = ITMMainEngine::InfiniTAM_IMAGE_ORIGINAL_DEPTH;
		...
		else
		{
			...
			uiEngine->outImageType[1] = ITMMainEngine::InfiniTAM_IMAGE_FREECAMERA_SHADED;
			...
			if (uiEngine->mainEngine->GetView() != NULL) {
				...
				uiEngine->outImage[1]->ChangeDims(uiEngine->mainEngine->GetView()->depth->noDims);
			}
	...
	const char* outFolder, ITMLibSettings::DeviceType deviceType)
{
	...
	float h1 = textHeight / (float)winSize.y, h2 = (1.f + h1) / 2;
	winReg[0] = Vector4f(0.0f, h1, 0.335f, 1.0f);   // Main render 0
	winReg[1] = Vector4f(0.335f, h1, 0.665f, 1.0f);	// Main render 1
	winReg[2] = Vector4f(0.665f, h2, 1.0f, 1.0f);   // Side sub window 2
	winReg[3] = Vector4f(0.665f, h1, 1.0f, h2);     // Side sub window 3
	...
	outImageType[2] = ITMMainEngine::InfiniTAM_IMAGE_ORIGINAL_DEPTH;
	outImageType[3] = ITMMainEngine::InfiniTAM_IMAGE_ORIGINAL_RGB;
	if (inputRGBImage->noDims == Vector2i(0, 0)) outImageType[3] = ITMMainEngine::InfiniTAM_IMAGE_UNKNOWN;
