...
ITMBasicEngine<TVoxel, TIndex>::ITMBasicEngine(const ITMLibSettings* settings, const ITMRGBDCalib& calib, Vector2i imgSize_rgb, Vector2i imgSize_d)
{
	...
	this->scene1 = new ITMScene<TVoxel, TIndex>(&settings->sceneParams, settings->swappingMode == ITMLibSettings::SWAPPINGMODE_ENABLED, memoryType);
	this->scene2 = new ITMScene<TVoxel, TIndex>(&settings->sceneParams, settings->swappingMode == ITMLibSettings::SWAPPINGMODE_ENABLED, memoryType);
	...
	denseMapper->ResetScene(scene1);
	denseMapper->ResetScene(scene2);

	imuCalibrator = new ITMIMUCalibrator_iPad();
	tracker = ITMTrackerFactory::Instance().Make(imgSize_rgb, imgSize_d, settings, lowLevelEngine, imuCalibrator, activeScene->sceneParams);
	...
	renderState_live = ITMRenderStateFactory<TIndex>::CreateRenderState(trackedImageSize, activeScene->sceneParams, memoryType);
...
void ITMBasicEngine<TVoxel, TIndex>::SaveSceneToMesh(const char* objFileName)
{
	...
	meshingEngine->MeshScene(mesh, activeScene);
...
void ITMBasicEngine<TVoxel, TIndex>::SaveToFile()
{
	...
	activeScene->SaveToDirectory(sceneOutputDirectory);
}
...
void ITMBasicEngine<TVoxel, TIndex>::LoadFromFile()
{
	...
	try // load scene
	{
		activeScene->LoadFromDirectory(sceneInputDirectory);
	}
	catch (std::runtime_error& e)
	{
		denseMapper->ResetScene(activeScene);
...
template<typename TVoxel, typename TIndex>
void ITMLib::ITMBasicEngine<TVoxel, TIndex>::switchActiveScene()
{
	if (activeScene_idx == 1) {
		activeScene = scene2;
		activeScene_idx = 2;
	}
	else {
...
template<typename TVoxel, typename TIndex>
void ITMLib::ITMBasicEngine<TVoxel, TIndex>::saveSceneBeforeSwitching()
{
	if (activeScene_idx == 1) {
		scene1->saveMe = true;
		scene2->saveMe = false;
...
ITMTrackingState::TrackingResult ITMBasicEngine<TVoxel, TIndex>::ProcessFrame(ITMUChar4Image* rgbImage, ITMShortImage* rawDepthImage, ITMIMUMeasurement* imuMeasurement)
{
	...
	if (settings->behaviourOnFailure == ITMLibSettings::FAILUREMODE_RELOCALISE)
	{
		...
		if (!hasAddedKeyframe && trackerResult == ITMTrackingState::TRACKING_FAILED)
		{
			...
			denseMapper->UpdateVisibleList(view, trackingState, activeScene, renderState_live, true);
			trackingController->Prepare(trackingState, activeScene, view, visualisationEngine, renderState_live);
	...
	if ((trackerResult == ITMTrackingState::TRACKING_GOOD || !trackingInitialised) && (fusionActive) && (relocalisationCount == 0)) {
		// fusion
		denseMapper->ProcessFrame(view, trackingState, activeScene, renderState_live);
		didFusion = true;
	...
	{
		if (!didFusion) denseMapper->UpdateVisibleList(view, trackingState, activeScene, renderState_live);
...
void ITMBasicEngine<TVoxel, TIndex>::GetImage(ITMUChar4Image* out, GetImageType getImageType, ORUtils::SE3Pose* pose, ITMIntrinsics* intrinsics)
{
	...
	case ITMBasicEngine::InfiniTAM_IMAGE_COLOUR_FROM_CONFIDENCE:
	{
		...
		visualisationEngine->RenderImage(activeScene, trackingState->pose_d, &view->calib.intrinsics_d, renderState_live, renderState_live->raycastImage, imageType, raycastType);
		visualisationEngine->updateDepthIntervals(activeScene, activeScene_idx == 1 ? scene2 : scene1, trackingState->pose_d, &view->calib.intrinsics_d, renderState_live, renderState_live->raycastImage, imageType, raycastType);
	...
	case ITMBasicEngine::InfiniTAM_IMAGE_FREECAMERA_COLOUR_FROM_CONFIDENCE:
	{
		...
		{
			renderState_freeview = ITMRenderStateFactory<TIndex>::CreateRenderState(out->noDims, activeScene->sceneParams, settings->GetMemoryType());
		}

		visualisationEngine->FindVisibleBlocks(activeScene, pose, intrinsics, renderState_freeview);
		visualisationEngine->CreateExpectedDepths(activeScene, pose, intrinsics, renderState_freeview);
		visualisationEngine->RenderImage(activeScene, pose, intrinsics, renderState_freeview, renderState_freeview->raycastImage, type);
