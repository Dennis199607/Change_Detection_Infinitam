...
template<class TVoxel, class TIndex, bool modifyVisibleEntries>
_CPU_AND_GPU_CODE_ inline bool castRayChange(DEVICEPTR(Vector4f)& pt_out, DEVICEPTR(uchar)* entriesVisibleType,
	int x, int y, const CONSTPTR(TVoxel)* voxelData, const CONSTPTR(typename TIndex::IndexData)* voxelIndex,
	Matrix4f invM, Vector4f invProjParams, float oneOverVoxelSize, float mu, const CONSTPTR(Vector2f)& viewFrustum_minmax, float* change_from, float* change_to, int* change_voxelTypes, int minX, int minY, int minZ, int maxX, int maxY, int maxZ)
{
	Vector4f pt_camera_f; Vector3f pt_block_s, pt_block_e, rayDirection, pt_result;
	bool pt_found;
	int vmIndex;
	...
	stepScale = mu * oneOverVoxelSize;
	float voxelSize = 1.0f / oneOverVoxelSize;
	...
	float change_length = 0;
	int sizeX = maxX - minX;
	int sizeY = maxY - minY;

	while (totalLength < totalLengthMax) {
		int index_x = (int)(pt_result.x - (float)minX);
		int index_y = (int)(pt_result.y - (float)minY);
		int index_z = (int)(pt_result.z - (float)minZ);

		// go through voxel tower with intervals at (index_x, index_y) and check in which interval the voxel lies
		if (index_x >= 0 && index_x < sizeX &&
			index_y >= 0 && index_y < sizeY) 
		{
			for (int i = 0; i < cuda_nrDepthIntervals; i++) {
				float from = change_from[index_x + (sizeX * index_y) + (sizeX * sizeY * i)];
				float to = change_to[index_x + (sizeX * index_y) + (sizeX * sizeY * i)];
				int voxelType = change_voxelTypes[index_x + (sizeX * index_y) + (sizeX * sizeY * i)];

				/*if (voxelType == CUDA_CHANGE) {
					change_found = true;
					change_length = totalLength;
				}*/

				if (pt_result.z >= from && pt_result.z < to) {
					if (voxelType == CUDA_CHANGE) {
						change_found = true;
						change_length = totalLength;
						break;
...
_CPU_AND_GPU_CODE_ inline void drawPixelRed(DEVICEPTR(Vector4u)& dest)
{
	dest = Vector4u((uchar)255.0f, (uchar)0.0f, (uchar)0.0f, (uchar)0.0f);
}
...
template<bool useSmoothing, bool flipNormals>
_CPU_AND_GPU_CODE_ inline void processPixel_Change(DEVICEPTR(Vector4u)* outRendering, const CONSTPTR(Vector4f)* pointsRay,
	const THREADPTR(Vector2i)& imgSize, const THREADPTR(int)& x, const THREADPTR(int)& y, float voxelSize, const THREADPTR(Vector3f)& lightSource)
{
	...
	bool surface = point.w > 0.0f;
	bool change = point.w < 0.0f;
	computeNormalAndAngle<useSmoothing, flipNormals>(surface, x, y, pointsRay, lightSource, voxelSize, imgSize, outNormal, angle);
	...
	if (change) {
		drawPixelRed(outRendering[locId]);
		return;
	}
