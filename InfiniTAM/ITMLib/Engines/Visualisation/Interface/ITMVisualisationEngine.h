...
namespace ITMLib
{
	...
	public:
		...
		*/
		...
		/** This will render the changes between the active scene and the old scene. */
		virtual void RenderChanges(const ITMScene<TVoxel, TIndex>* scene, const ORUtils::SE3Pose* pose, const ITMIntrinsics* intrinsics,
			...
			RenderImageType type = RENDER_SHADED_GREYSCALE, RenderRaycastSelection raycastType = RENDER_FROM_NEW_RAYCAST) const = 0;
		...
		*/
		...
		/** This will update the change detection between the active scene and the old scene. */
		virtual void updateDepthIntervals(ITMScene<TVoxel, ITMVoxelBlockHash>* activeScene, ITMScene<TVoxel, ITMVoxelBlockHash>* oldScene, const ORUtils::SE3Pose* pose, const ITMIntrinsics* intrinsics,
			const ITMRenderState* renderState, ITMUChar4Image* outputImage, RenderImageType type = RENDER_SHADED_GREYSCALE, RenderRaycastSelection raycastType = RENDER_FROM_NEW_RAYCAST) = 0;
	};
}
