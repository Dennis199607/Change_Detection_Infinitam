...
namespace ITMLib
{
	...
	template<class TVoxel, class TIndex, bool modifyVisibleEntries>
	__global__ void genericRaycastChange_device(Vector4f* out_ptsRay, uchar* entriesVisibleType, const TVoxel* voxelData,
		const typename TIndex::IndexData* voxelIndex, Vector2i imgSize, Matrix4f invM, Vector4f invProjParams,
		float oneOverVoxelSize, const Vector2f* minmaximg, float mu, float* change_from, float* change_to, int* change_voxelTypes, int minX, int minY, int minZ, int maxX, int maxY, int maxZ)
	{
		int x = (threadIdx.x + blockIdx.x * blockDim.x), y = (threadIdx.y + blockIdx.y * blockDim.y);
		...
		castRayChange<TVoxel, TIndex, modifyVisibleEntries>(out_ptsRay[locId], entriesVisibleType, x, y, voxelData, voxelIndex, invM, invProjParams, oneOverVoxelSize, mu, minmaximg[locId2], change_from, change_to, change_voxelTypes, minX, minY, minZ, maxX, maxY, maxZ);
	}
	...
	template<bool flipNormals>
	__global__ void renderChange_device(Vector4u* outRendering, const Vector4f* pointsRay, float voxelSize, Vector2i imgSize, Vector3f lightSource)
	{
		int x = (threadIdx.x + blockIdx.x * blockDim.x), y = (threadIdx.y + blockIdx.y * blockDim.y);

		if (x >= imgSize.x || y >= imgSize.y) return;

		processPixel_Change<true, flipNormals>(outRendering, pointsRay, imgSize, x, y, voxelSize, lightSource);
	}
